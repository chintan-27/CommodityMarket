...
class Tform(forms.form):
    ...
    city= forms.CharField(max_length=30,widget=forms.TextField(attrs={'class':'d-form form-control'}))

     class Meta:
         model = TruckDrivers
         fields = {'trucktype','status','city'}

class Dform(forms.form): forms.CharField(max_length=30,widget=forms.Select(choices=TRUCK_TYPE,attrs={'class':'d-form form-control'}))
    STATUS_TYPE = (
    ('Available','Available'),
    ('Unavailable','Unavailable'),
    ('Busy','Busy'),
